			...
			function TableDetail() {
				...
				var _TableHeadServiceCaller = new TableHeadServiceCaller();function Initialise() {
					_vm = new ViewModel();
					setTimeout(function() {
						ko.applyBindings(_vm, $("#TableDetailPage")[0]);
					}, 60);
					GetAll();GetAllTableHead();};function ViewModel() {
					var self = this;
					self.MaintObj = ko.observable(new TableDetailViewModel());
					self.IsAdd = ko.observable(true);
					self.TableDetailList = ko.observableArray([]);self.TableHeadList = ko.observableArray([]);self.SetMaintObj = function(data) {
			        ...
			        self.Save = function (vm, e) {
			            if (e.isDefaultPrevented()) {
			                ...
			                e.preventDefault();            
			                if (self.IsAdd()) {
			                    function callback_Create(response) {_vm.TableDetailList.push(new TableDetailViewModel(response.TableDetailList[0], Modify, Delete,_vm.TableHeadList));$("#create-edit-TableDetail-modal").modal("hide");
			                        self.ClearMaintObj();
			                    }
				...
				function GetAll() {
					_TableDetailServiceCaller.GetAll(function(response) {
						$(response.TableDetailList).each(function(idx, el) {_vm.TableDetailList.push(new TableDetailViewModel(el, Modify, Delete,_vm.TableHeadList));});
				        setTimeout(() => {
	            			$('.dropdown-toggle-maint').dropdown();
	    				}, 200);
					});
				};function GetAllTableHead() {
